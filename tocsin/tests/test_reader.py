...
from tocsin.reader import fq_encoder, fq_reader, fq_encoder_alt
...
class TestCythonReaders(TocsinTestCase):
    ...
    def test_fq_encoder_alt_default_len_and_shorter_than_longest(self):
        demo_file = self.get_data_path('small_test_fastq_02.fq')
        lines = fq_encoder_alt(bytes(demo_file, 'utf-8'), 6)
        expected_reads = [
            # AACGT, CGGGAA, AGCGCCTT
            [0, 0, 1, 2, 3, -1],
            [1, 2, 2, 2, 0, 0],
